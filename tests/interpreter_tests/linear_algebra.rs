...
mod fit {
  ...
  fn linear_fit_explicit_xy_pairs() {
    assert_eq!(
      ...
      "0.1864406779661019 + 0.6949152542372881*x"
    );
  }
  ...
  fn quadratic_fit() {
    assert_eq!(
      ...
      "0.2199999999999992 + 1.08*x - 4.450793765416313*^-17*x^2"
    );
  }
  ...
  fn sin_basis() {
    assert_eq!(
      ...
      "3.220973081736608 - 1.936118011549195*Sin[x]"
    );
  }
  ...
  fn log_factorial_fit() {
    assert_eq!(
      interpret("Fit[N[Log[Table[Factorial[n], {n, 1, 20}]]], {1, x, x^2}, x]")
        .unwrap(),
      "-2.029632664074299 + 1.179018356536343*x + 0.05311657293874288*x^2"
    );
  }

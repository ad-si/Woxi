# Logical Operators and Conditionals

Boolean combinators and the `If` conditional.

- [`And`](logic/And.md)
- [`Or`](logic/Or.md)
- [`Not`](logic/Not.md)
- [`Xor`](logic/Xor.md)
- [`Nand`](logic/Nand.md)
- [`Nor`](logic/Nor.md)
- [`Implies`](logic/Implies.md)
- [`If`](logic/If.md)
- [`Boole`](logic/Boole.md)
- [`TrueQ`](logic/TrueQ.md)
- [`BooleanQ`](logic/BooleanQ.md)
- [`SameQ`](logic/SameQ.md)
- [`UnsameQ`](logic/UnsameQ.md)
- [`MatchQ`](logic/MatchQ.md)

## Additional Functions

- [`BooleanConvert`](logic/BooleanConvert.md)
- [`BooleanMinimize`](logic/BooleanMinimize.md)
- [`BooleanTable`](logic/BooleanTable.md)
- [`LogicalExpand`](logic/LogicalExpand.md)
- [`TautologyQ`](logic/TautologyQ.md)
- [`VectorLess`](logic/VectorLess.md)
- [`VectorLessEqual`](logic/VectorLessEqual.md)
- [`Xnor`](logic/Xnor.md)
- [`Equivalent`](logic/Equivalent.md)
- [`True`](logic/True.md)

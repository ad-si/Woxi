...
/// The barycentric coordinates of `point` relative to the affine coordinate
/// system spanned by `simplex`, an (n+1)-point simplex in n-dimensional
/// space — the algorithm behind the Wolfram Function Repository resource
/// `ResourceFunction["BarycentricCoordinates"]`. Not a kernel builtin in
/// real Wolfram (only reachable through `ResourceFunction`), so this is
/// dispatched from `apply_curried_call` rather than exposed as a bare name.
pub(crate) fn compute_barycentric_coordinates(
  simplex: &Expr,
  point: &Expr,
) -> Result<Expr, InterpreterError> {
  let uneval = || {
    Ok(call(
      ...
      vec![simplex.clone(), point.clone()],
    ))
  };
  let (Expr::List(verts), Expr::List(p)) = (simplex, point) else {
    return uneval();
  };
  let vcoords: Vec<&[Expr]> = verts
    .iter()
    ...
    .collect();
  if vcoords.len() != verts.len() || vcoords.is_empty() {
    return uneval();
  }
  // n+1 vertices span an n-dimensional simplex; every vertex and the query
  // point must have that same dimension.
  let n = vcoords.len();
  let d = n - 1;
  if d == 0 || vcoords.iter().any(|c| c.len() != d) || p.len() != d {
    return uneval();
  }

  // Exact path when every coordinate is an integer or rational: solve
  // T . lambda = point - last for the first d coordinates (T's columns are
  // vertex[j] - last), then the final coordinate is 1 - sum(lambda).
  let exact = (|| -> Option<Vec<BigRat>> {
    let vr: Vec<Vec<BigRat>> = vcoords
      .iter()
      .map(|c| {
        c.iter()
          .map(|e| expr_to_rational(e).map(BigRat::from_pair))
          .collect::<Option<Vec<_>>>()
      })
      .collect::<Option<Vec<_>>>()?;
    let pr: Vec<BigRat> = p
    ...
    for i in 0..d {
      let row: Vec<BigRat> =
        vr.iter().take(d).map(|vj| vj[i].sub(&last[i])).collect();
      a.push(row);
      b.push(pr[i].sub(&last[i]));
    }
    let mut lambda = solve_bigrat_system(a, &b)?;
    let sum = lambda.iter().fold(BigRat::zero(), |acc, x| acc.add(x));
    lambda.push(BigRat::from_pair((1, 1)).sub(&sum));
    Some(lambda)
  })();
  if let Some(lambda) = exact {
    let items: Result<Vec<Expr>, InterpreterError> =
      lambda.iter().map(BigRat::to_expr).collect();
    return Ok(Expr::List(items?.into()));
  }

  // Float path.
  let vf: Option<Vec<Vec<f64>>> = vcoords
    .iter()
    .map(|c| {
      c.iter()
        .map(crate::functions::math_ast::try_eval_to_f64)
  ...
  let Some(mut lambda) = solve_float_system(a, b) else {
    return uneval();
  };

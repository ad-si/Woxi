...
pub fn dispatch_polynomial_functions(
  ...
  match name {
    ...
    "MonomialList" if args.len() == 2 || args.len() == 3 => {
    ...
    "MinimalPolynomial" if args.len() == 1 => {
      return Some(
        crate::functions::polynomial_ast::minimal_polynomial_pure_ast(&args[0]),
      );
    }
    ...
    "PowerSymmetricPolynomial" if args.len() == 1 || args.len() == 2 => {
      return Some(

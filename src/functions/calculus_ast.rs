      ...
      match name.as_str() {
        ...
        "ArcTan" if args.len() == 2 => {
          ...
          let denom = Expr::BinaryOp {
            op: BinaryOperator::Plus,
            left: Box::new(pow2(u.clone(), Expr::Integer(2))),
            right: Box::new(pow2(v.clone(), Expr::Integer(2))),
          };
...
fn make_fresnel_antiderivative(
  ...
  let result = Expr::FunctionCall {
    name: "Times".to_string(),
    args: vec![sqrt_pi_2, pow2(sqrt_a, Expr::Integer(-1)), fresnel].into(),
  };
            ...
            && n == var
          {
            ...
            let correction = match name.as_str() {
              ...
              _ => Expr::BinaryOp {
                ...
                left: Box::new(Expr::FunctionCall {
                  name: "Log".to_string(),
                  args: vec![minus2(Expr::Integer(1), x_sq)].into(),
                }),
                right: Box::new(Expr::Integer(2)),
              },
            };
...
fn try_exponential_delta(expr: &Expr, var: &str, step: &Expr) -> Option<Expr> {
  ...
  let delta_exp = Expr::FunctionCall {
    ...
    args: vec![minus2(shifted_exp, exponent.clone())].into(),
  };
...
fn try_trig_delta(expr: &Expr, var: &str, step: &Expr) -> Option<Expr> {
  ...
  let half_delta_raw = Expr::BinaryOp {
    ...
    left: Box::new(Expr::FunctionCall {
      name: "Expand".to_string(),
      args: vec![minus2(shifted_arg.clone(), arg.clone())].into(),
    }),
    right: Box::new(Expr::Integer(2)),
  };
  let half_delta =

...
fn extract_comparison(expr: &Expr) -> Option<(Expr, Expr, CompOp)> {
  match expr {
    Expr::Comparison {
      operands,
      operators,
...
fn collect_and_constraints(expr: &Expr, out: &mut Vec<Expr>) {
  match expr {
    Expr::BinaryOp {
      op: BinaryOperator::And,
      left,
...
fn make_equality(lhs: &Expr, rhs: &Expr) -> Expr {
  Expr::Comparison {
    operands: vec![lhs.clone(), rhs.clone()],
    operators: vec![crate::syntax::ComparisonOp::Equal],
  }
...
fn make_comparison(lhs: &Expr, rhs: &Expr, op: CompOp) -> Expr {
...
fn build_or(mut terms: Vec<Expr>) -> Expr {
  // Filter out False
...
fn build_sum_from_parts(parts: &[Expr]) -> Expr {
  if parts.is_empty() {
    Expr::Integer(0)
  } else {
    parts
...
fn or_results(a: &Expr, b: &Expr) -> Expr {
  match (a, b) {
...
fn and_results(a: &Expr, b: &Expr) -> Expr {
  match (a, b) {
...
fn compare_exprs(a: &Expr, b: &Expr) -> std::cmp::Ordering {
...
fn build_mul(a: &Expr, b: &Expr) -> Expr {
  Expr::FunctionCall {
    name: "Times".to_string(),
...
fn build_sub(a: &Expr, b: &Expr) -> Expr {
  Expr::FunctionCall {
    name: "Plus".to_string(),
    args: vec![
      a.clone(),
